-- ============================================================================
-- FIX: Admin User ID Test Mode - Handle NULL auth.uid() in SQL Editor
-- ============================================================================
-- Issue: admin_audit_log.admin_user_id is NOT NULL but auth.uid() returns NULL
--        in SQL Editor (service role context)
-- Fix: Add 'service_role' fallback for test mode
-- ============================================================================
...
DECLARE
  ...
  v_rack_names TEXT[] := '{}';
  v_admin_user_id TEXT;  -- ✅ NEW: Local variable for admin ID
  v_result JSON;
BEGIN
  -- ✅ FIX: Handle NULL auth.uid() in test mode (SQL Editor)
  v_admin_user_id := COALESCE(
    p_admin_user_id,
    auth.uid()::text,
    'service_role'  -- Fallback for SQL Editor testing
  );
  ...
  -- ✅ FIX: Use v_admin_user_id instead of COALESCE inline
  INSERT INTO admin_audit_log (admin_user_id, action, entity_type, entity_id, details, created_at)
  VALUES (
    v_admin_user_id,  -- Now guaranteed to be non-NULL
    'APPROVE_REQUEST',
    'storage_request',
    p_request_id,
    json_build_object(
...
'Atomically approves storage request. admin_user_id defaults to service_role in test mode.';

COMMENT ON FUNCTION reject_storage_request_atomic IS
'Atomically rejects storage request. admin_user_id defaults to service_role in test mode.';
