-- ============================================================================
-- MIGRATION FIX: Correct column name in mark_load_completed function
-- ============================================================================
--
-- Issue: Function referenced "created_at" but trucking_documents uses "uploaded_at"
-- Error: Load completion transaction failed [42703]: column "created_at" does not exist
-- Fix: Change ORDER BY created_at DESC to ORDER BY uploaded_at DESC (line 134)
--
-- This CREATE OR REPLACE will update the existing function with the correction
-- ============================================================================
...
BEGIN
    ...
    ORDER BY uploaded_at DESC  -- FIXED: Changed from created_at to uploaded_at
    LIMIT 1;
...
'Atomically completes a trucking load by updating status, creating inventory records from manifest data, and updating rack occupancy. All operations are wrapped in a transaction to ensure data integrity. FIXED: Uses uploaded_at instead of created_at for trucking_documents query.';
