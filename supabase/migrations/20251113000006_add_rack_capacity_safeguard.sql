-- ============================================================================
-- MIGRATION: Add Rack Capacity Safeguard to Load Completion
-- ============================================================================
--
-- Purpose: Prevent rack occupancy from exceeding capacity during load completion
--
-- Problem: Currently, mark_load_completed_and_create_inventory increments
--          rack occupancy without checking if it exceeds capacity.
--
-- Solution: Add capacity validation before updating rack occupancy.
--           Throw clear error if completion would exceed capacity.
--
-- Safety: This prevents physical over-allocation of rack space.
--
-- ============================================================================
...
BEGIN
    ...
    SELECT
        ...
        COALESCE(avg_joint_length_ft, 30.0)
    INTO
...
-- Add comment documenting the safeguard
COMMENT ON FUNCTION public.mark_load_completed_and_create_inventory IS
'Atomically completes a trucking load with capacity safeguard. Validates rack capacity before incrementing occupancy to prevent over-allocation. Handles both loads with manifest data (detailed inventory) and legacy loads without manifests (simplified inventory). Updated 2025-11-13 with capacity safeguard.';
